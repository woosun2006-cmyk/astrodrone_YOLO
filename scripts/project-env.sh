...
# The model source is shared across platforms, but TensorRT engines are not.
# Build/load one engine per GPU architecture and runtime.
...
case "$(uname -m)" in
  x86_64)
    ...
    YOLO_ENGINE_PATH="${YOLO_ENGINE_PATH:-$YOLO_DIR/best_v5_wsl.engine}"
    YOLO_LIVE_BUILD_DIR="${YOLO_LIVE_BUILD_DIR:-$REPO_ROOT/build/yolo_live-x86_64}"
    ;;
  aarch64|arm64)
    YOLO_PLATFORM="${YOLO_PLATFORM:-aarch64}"
    YOLO_ENGINE_PATH="${YOLO_ENGINE_PATH:-$YOLO_DIR/best_v5_jetson.engine}"
    YOLO_LIVE_BUILD_DIR="${YOLO_LIVE_BUILD_DIR:-$REPO_ROOT/build/yolo_live-aarch64}"
    ;;
  *)
    YOLO_PLATFORM="${YOLO_PLATFORM:-$(uname -m)}"
    YOLO_ENGINE_PATH="${YOLO_ENGINE_PATH:-$YOLO_DIR/best_v5_${YOLO_PLATFORM}.engine}"
...
# Endpoint contract. C++ control/sensing addresses are defined by setting/
# port.yaml; these names document the corresponding SITL/router endpoints.
...
MAVPROXY_CONTROL_ENDPOINT="${MAVPROXY_CONTROL_ENDPOINT:-udp:127.0.0.1:14550}"
MAVPROXY_TELEMETRY_ENDPOINT="${MAVPROXY_TELEMETRY_ENDPOINT:-udp:127.0.0.1:14551}"
# Use a separate router fan-out because UDP subscribers cannot safely share
# one bound telemetry socket with target-distance/control.
MAVPROXY_GCS_TELEMETRY_ENDPOINT="${MAVPROXY_GCS_TELEMETRY_ENDPOINT:-udp:127.0.0.1:14553}"
SITL_CONTROL_ENDPOINT="${SITL_CONTROL_ENDPOINT:-$MAVPROXY_CONTROL_ENDPOINT}"
SITL_SENSOR_ENDPOINT="${SITL_SENSOR_ENDPOINT:-$MAVPROXY_TELEMETRY_ENDPOINT}"
MISSION_PLANNER_ENDPOINT="${MISSION_PLANNER_ENDPOINT:-udp:127.0.0.1:14552}"
...
export SITL_CONTROL_ENDPOINT SITL_SENSOR_ENDPOINT
export MAVPROXY_CONTROL_ENDPOINT MAVPROXY_TELEMETRY_ENDPOINT MAVPROXY_GCS_TELEMETRY_ENDPOINT
export MISSION_PLANNER_ENDPOINT MISSION_UPLOAD_ENDPOINT TARGET_UDP_ENDPOINT YOLO_HTTP_ENDPOINT YOLO_ENGINE_PATH YOLO_ONNX_PATH YOLO_MODEL_PATH GAZEBO_FDM_ENDPOINT
...
print_dependency_report() {
  ...
  printf 'C++ control=%s\nC++ sensor=%s\n' "$SITL_CONTROL_ENDPOINT" "$SITL_SENSOR_ENDPOINT"
  printf 'MAVProxy control=%s\ntelemetry=%s\nMission Planner=%s\nmission upload=%s\ntarget UDP=%s\nYOLO HTTP=%s\n' \
    "$MAVPROXY_CONTROL_ENDPOINT" "$MAVPROXY_TELEMETRY_ENDPOINT" \
    "$MISSION_PLANNER_ENDPOINT" "$MISSION_UPLOAD_ENDPOINT" \
    "$TARGET_UDP_ENDPOINT" "$YOLO_HTTP_ENDPOINT"
}
