#pragma once
// NOTE: EKF_STATUS_REPORT (id 193) is defined in ArduPilot's ardupilotmega.xml,
// not in common.xml, so it isn't part of the "common" dialect subset the rest
// of this directory was vendored from. Generated with pymavlink's mavgen
// (language=C) from ardupilotmega.xml so the struct layout/CRC_EXTRA/lengths
// are authoritative, then dropped in here and wired into common.h's
// MAVLINK_MESSAGE_CRCS/MAVLINK_MESSAGE_INFO tables and #include list -- see
// setting/safety.yaml's vehicle_health_limit comment for why we need it
// (EKF variance-based failsafe, matching astroquad/uav-onboard's approach).
