#include "diagonal_approach_guidance.hpp"
...
DiagonalApproachGuidance::DiagonalApproachGuidance(
    DiagonalApproachGuidanceConfig config)
    ...
    if (config_.desired_path_angle_rad <= 0.0) {
        config_.desired_path_angle_rad = config_.max_vector_speed > 1e-12
...
double DiagonalApproachGuidance::apply_deadband(double value, bool& active) const {
...
void DiagonalApproachGuidance::limit_horizontal(double& vx, double& vy) const {
...
GuidanceDiagnostics DiagonalApproachGuidance::update(
    double x_px, double y_px, double altitude_m, double distance_m,
    bool fresh_confirmed_target, double dt_sec, double observation_age_ms) {
    ...
    const bool final_centering = distance_m <= config_.stop_distance_m;
    ...
    // Keep approaching while the confirmed target remains inside the camera's
    ...
    const double minimum_safety_scale =
        !final_centering && horizontal_before_safety > 1e-12
            ...
            : 0.0;
    const double safety_scale =
        in_safe_fov ? std::max(output.descent_margin, minimum_safety_scale) : 0.0;
...
GuidanceDiagnostics DiagonalApproachGuidance::force_zero(
...
void DiagonalApproachGuidance::reset_history() {
    initialized_ = false;
